...
async function createNewAdminUser() {
    try {
        ...
        const checkResult = await pool.query("SELECT id FROM users WHERE email = $1", ["admin@bubblescafe.com"]);
        if (checkResult.rows.length > 0) {
            console.log("Admin user already exists, updating password...");
            // Update the password
            await pool.query("UPDATE users SET password_hash = $1 WHERE email = $2", [hashedPassword, "admin@bubblescafe.com"]);
            console.log("Admin password updated successfully");
            return { id: checkResult.rows[0].id, updated: true };
        }
        // Create new admin user
        const result = await pool.query(`INSERT INTO users (username, email, password_hash, is_admin, created_at) 
       VALUES ($1, $2, $3, $4, NOW()) 
       RETURNING id, username, email, is_admin as "isAdmin", created_at as "createdAt"`, ["admin", "admin@bubblescafe.com", hashedPassword, true]);
