 ...
 */
export function registerRecommendationsRoutes(app, storage) {
        ...
        try {
            ...
            if (historyPostIds.size === 0) {
                ...
                if (preferredThemes.length > 0) {
                    const themeConditions = preferredThemes.map(theme => or(like(posts.themeCategory, `%${theme}%`), sql `${posts.metadata}->>'themeCategory' LIKE ${`%${theme}%`}`));
                    const finalQuery = query.where(themeConditions.reduce((acc, condition) => or(acc, condition)));
            ...
            const contentBasedRecommendations = await safeDbOperation(async () => {
                ...
                allThemes.map(theme => or(like(posts.themeCategory, `%${theme}%`), sql `${posts.metadata}->>'themeCategory' LIKE ${`%${theme}%`}`)).reduce((acc, condition) => or(acc, condition), sql `1=0`)))
    ...
    app.get("/api/recommendations/personalized", async (req, res) => {
        ...
        try {
            ...
            const recommendedPosts = await storage.getPersonalizedRecommendations(userId, preferredThemes, limit);
     ...
     */
    app.get("/api/recommendations/direct", async (req, res) => {
