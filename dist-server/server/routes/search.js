...
const trendingQueries = new Map();
function makeCacheKey(params) {
    return JSON.stringify(params);
}
function recordTrending(query) {
    try {
...
router.get('/', async (req, res) => {
    try {
        ...
        const { q, types = 'posts,pages,comments,legal,settings', limit = '20', page = '1', from, // can be number of days or ISO date
        category } = req.query;
        ...
        const resultLimit = Math.min(Math.max(parseInt(limit, 10) || 20, 1), 50); // Between 1 and 50
        ...
        if (contentTypes.includes('posts')) {
            ...
            if (category && typeof category === 'string') {
                ...
                allPosts = allPosts.filter((p) => (p.themeCategory || '').toLowerCase() === cat);
            }
            let postResults = allPosts
                ...
                searchTerms.forEach(term => {
                    ...
                    if (contextMatches && contextMatches.length > 0) {
                        ...
                        contextMatches.slice(0, 3).forEach((context) => {
                            matches.push({
                                text: term,
            ...
            if (fromDate) {
                postResults = postResults.filter((r) => new Date(r.createdAt || 0) >= fromDate);
            }
        ...
        if (searchOptions.includePages) {
            try {
                ...
                let pagePosts = allPosts.filter((post) => post.isSecret === true);
                if (fromDate) {
                    pagePosts = pagePosts.filter((p) => new Date(p.createdAt || 0) >= fromDate);
                }
        ...
        if (searchOptions.includeComments) {
            try {
                ...
                if (fromDate) {
                    commentResults = commentResults.filter((r) => new Date(r.createdAt || 0) >= fromDate);
                }
    ...
    try {
        ...
        if (remaining > 0) {
            contentMatches = allPosts
                .filter((p) => !(p.title || '').toLowerCase().includes(search))
                .filter((p) => (p.content || '').toLowerCase().includes(search))
                .slice(0, remaining);
        }
        const combined = [...titleMatches, ...contentMatches];
        const suggestions = combined.map((p) => ({
            id: p.id,
