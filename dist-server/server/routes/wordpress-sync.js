...
export function registerWordPressSyncRoutes(app) {
     ...
     */
    app.get('/api/wordpress/status', (_req, res) => {
     ...
     */
    app.get('/api/wordpress/status-check', async (_req, res) => {
        // Set proper Content-Type to ensure JSON response
        res.setHeader('Content-Type', 'application/json');
        try {
     ...
     */
    app.post('/api/wordpress/sync', simpleRateLimit(), requireAdmin, async (req, res) => {
        logEvent('Manual WordPress sync triggered via API', { user: req.user });
        if (syncInProgress) {
     ...
     */
    app.post('/api/wordpress/sync/:postId', simpleRateLimit(), requireAdmin, async (req, res) => {
        // Validate input
