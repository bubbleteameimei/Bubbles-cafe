 ...
 */
anonymousBookmarkRouter.get('/:postId', async (req, res) => {
    try {
 ...
 */
anonymousBookmarkRouter.get('/', async (req, res) => {
    try {
        // Initialize if doesn't exist
        if (!req.session.anonymousBookmarks) {
            req.session.anonymousBookmarks = {};
        ...
        bookmarks.sort((a, b) => new Date(b?.createdAt || 0).getTime() - new Date(a?.createdAt || 0).getTime());
        res.json(bookmarks);
        return;
    }
    catch (error) {
 ...
 */
anonymousBookmarkRouter.post('/', async (req, res) => {
    try {
 ...
 */
anonymousBookmarkRouter.patch('/:postId', async (req, res) => {
    try {
        const postId = parseInt(req.params.postId);
        ...
        if (notes !== undefined)
            req.session.anonymousBookmarks[postId].notes = notes;
        if (tags !== undefined)
            req.session.anonymousBookmarks[postId].tags = tags;
        if (lastPosition !== undefined)
            req.session.anonymousBookmarks[postId].lastPosition = lastPosition;
 ...
 */
anonymousBookmarkRouter.delete('/:postId', async (req, res) => {
    try {
        const postId = parseInt(req.params.postId);
        if (isNaN(postId)) {
            res.status(400).json({ error: "Invalid post ID" });
 ...
 */
export function registerBookmarkRoutes(app) {
    ...
    const bypassSecondaryCSRF = (req, res, next) => {
