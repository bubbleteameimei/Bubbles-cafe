...
import { z } from "zod";
import { updateCommentSchema } from "@shared/schema";
...
function getUserKey(req) {
    ...
    return req.sessionID ? `anon:${req.sessionID}` : 'anon';
}
// GET /api/posts/:postId/comments - list comments for a post
router.get('/posts/:postId/comments', apiRateLimiter, validateParams(postIdSchema), asyncHandler(async (req, res) => {
    ...
    const enhanced = comments.map((c) => ({
        ...
        approved: c.approved === undefined ? Boolean(c.is_approved) : Boolean(c.approved),
        isOwner: c.metadata && c.metadata.ownerKey
            ? String(c.metadata.ownerKey) === userKey
            : false
    }));
    res.json(enhanced);
}));
// POST /api/posts/:postId/comments - create a new comment or reply (when parentId provided)
router.post('/posts/:postId/comments', apiRateLimiter, validateParams(postIdSchema), validateBody(createCommentBodySchema), asyncHandler(async (req, res) => {
    const postId = Number(req.params.postId);
    const body = req.body;
    const userKey = getUserKey(req);
    ...
    const inferredAuthor = body.author && body.author.trim().length > 0
        ? body.author.trim()
        : (req.user?.username || (req.user?.id ? 'User' : 'Anonymous'));
    const insert = {
    ...
    const created = await storage.createComment(insert);
    // Add isOwner to response for immediate UI use
    res.status(201).json({ ...created, approved: created.is_approved === true, isOwner: true });
}));
// POST /api/comments/:id/vote - upvote/downvote by user or anonymous session
router.post('/comments/:id/vote', apiRateLimiter, validateParams(commentIdSchema), validateBody(voteBodySchema), asyncHandler(async (req, res) => {
...
router.post('/comments/:id/flag', apiRateLimiter, validateParams(commentIdSchema), validateBody(flagBodySchema), asyncHandler(async (req, res) => {
    const commentId = Number(req.params.id);
...
router.patch('/comments/:id', apiRateLimiter, validateParams(commentIdSchema), validateBody(updateCommentBodySchema), asyncHandler(async (req, res) => {
    const commentId = Number(req.params.id);
    ...
    if (!existing)
        throw createError('Comment not found', 404);
    const isOwner = existing.metadata?.ownerKey && String(existing.metadata.ownerKey) === userKey;
    if (!isOwner)
        throw createError('Not allowed to edit this comment', 403);
...
router.delete('/comments/:id', apiRateLimiter, validateParams(commentIdSchema), asyncHandler(async (req, res) => {
    const commentId = Number(req.params.id);
    const userKey = getUserKey(req);
    const existing = await storage.getComment(commentId);
    if (!existing)
    ...
    const isOwner = existing.metadata?.ownerKey && String(existing.metadata.ownerKey) === userKey;
    if (!isOwner)
        throw createError('Not allowed to delete this comment', 403);
