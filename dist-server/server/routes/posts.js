...
router.get('/', apiRateLimiter, validateQuery(postQuerySchema), asyncHandler(async (req, res) => {
    const { page, limit, category, search } = req.query;
    try {
        const result = await storage.getPosts(Number(page), Number(limit), { category, search });
...
router.get('/:id', apiRateLimiter, validateParams(postIdSchema), asyncHandler(async (req, res) => {
    const { id } = req.params;
    try {
        const post = await storage.getPostById
            ? storage.getPostById(Number(id))
...
router.get('/slug/:slug', apiRateLimiter, asyncHandler(async (req, res) => {
...
router.get('/community', apiRateLimiter, asyncHandler(async (req, res) => {
    try {
        ...
        try {
            const counts = await storage.getPostsCount?.();
...
router.get('/admin/themes', apiRateLimiter, asyncHandler(async (_req, res) => {
    try {
        ...
        const data = rows.map((r) => ({
            ...
            themeIcon: r.metadata?.themeIcon,
            theme_icon: r.metadata?.themeIcon,
            metadata: r.metadata
        }));
        res.json(data);
    }
    catch (_error) {
        postsLogger.error('Error retrieving admin theme list', { error: _error?.message ?? String(_error) });
...
router.patch('/:id/theme', apiRateLimiter, validateParams(postIdSchema), asyncHandler(async (req, res) => {
    if (!req.user || !req.user.isAdmin) {
        throw createError.forbidden('Admin privileges required');
    }
    const { id } = req.params;
    const { themeCategory, theme_category, themeIcon, icon } = req.body || {};
    const newCategory = themeCategory || theme_category;
    const newIcon = themeIcon || icon;
    try {
        // Load existing post
        const existing = await storage.getPostById
            ? storage.getPostById(Number(id))
            : storage.getPost(String(id));
        if (!existing)
            throw createError.notFound('Post not found');
        ...
        if (newCategory)
            updatePayload.themeCategory = String(newCategory);
...
router.post('/', apiRateLimiter, validateBody(insertPostSchema), asyncHandler(async (req, res) => {
    if (!req.user) {
...
router.put('/:id', apiRateLimiter, validateParams(postIdSchema), validateBody(updatePostSchema), asyncHandler(async (req, res) => {
    if (!req.user) {
        throw createError.unauthorized('Authentication required');
    }
    const { id } = req.params;
    try {
        ...
        const existingPost = await (storage.getPostById
            ? storage.getPostById(Number(id))
            : storage.getPost(String(id)));
        if (!existingPost) {
            throw createError.notFound('Post not found');
...
router.put('/:id/hide', apiRateLimiter, validateParams(postIdSchema), asyncHandler(async (req, res) => {
    if (!req.user) {
        throw createError.unauthorized('Authentication required');
    }
    const { id } = req.params;
    try {
        ...
        const updated = await storage.updatePost(Number(id), { metadata: { ...existingPost.metadata || {}, isHidden: true } });
...
router.post('/:id/like', apiRateLimiter, validateParams(postIdSchema), asyncHandler(async (req, res) => {
    const { id } = req.params;
    try {
        await storage.updatePostReaction(Number(id), { isLike: true, sessionId: req.sessionID });
        const counts = await storage.getPostLikeCounts(Number(id));
...
router.post('/:id/flag', apiRateLimiter, validateParams(postIdSchema), asyncHandler(async (req, res) => {
    const { id } = req.params;
    const reason = typeof req.body?.reason === 'string' ? req.body.reason : 'unspecified';
    try {
        const reporterId = req.user?.id ?? 0;
...
router.delete('/:id', apiRateLimiter, validateParams(postIdSchema), asyncHandler(async (req, res) => {
    if (!req.user) {
        throw createError.unauthorized('Authentication required');
    }
    const { id } = req.params;
