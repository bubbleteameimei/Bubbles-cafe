...
export function registerUserFeedbackRoutes(app, storage) {
    /**
    ...
    app.get('/api/user/feedback', async (req, res) => {
        try {
            ...
            const userFeedback = allFeedback.filter(feedback => feedback.userId === userId);
            return res.status(200).json({
     ...
     */
    app.get('/api/user/feedback/stats', async (req, res) => {
        try {
            const { isAuthenticated, userId } = checkAuthentication(req);
            ...
            const stats = {
                ...
                reviewed: userFeedback.filter((item) => item.status === 'reviewed').length,
                resolved: userFeedback.filter((item) => item.status === 'resolved').length,
                rejected: userFeedback.filter((item) => item.status === 'rejected').length,
                responseRate: userFeedback.length > 0
                    ? (userFeedback.filter((item) => item.metadata && item.metadata.adminResponse).length / userFeedback.length) * 100
                    : 0
            };
            return res.status(200).json({
                stats,
     ...
     */
    app.get('/api/user/feedback/:id', async (req, res) => {
        try {
            const { isAuthenticated, userId } = checkAuthentication(req);
