...
export class WordPressAPISync {
    baseUrl = (process.env.WORDPRESS_API_URL || process.env.WORDPRESS_API || 'https://public-api.wordpress.com/wp/v2/sites/bubbleteameimei.wordpress.com');
    batchSize = 20;
    async syncAllPosts() {
        ...
        while (hasMore) {
            try {
                ...
                const response = await fetch(`${this.baseUrl}/posts?page=${page}&per_page=${this.batchSize}&status=publish&_fields=id,date,slug,title,content,excerpt,author,categories,tags,featured_media,status,type,modified`);
                if (!response.ok) {
    ...
    async syncSinglePost(wpPost, authorId) {
    ...
    async ensureUniqueSlug(baseSlug, wpId) {
        let slug = baseSlug;
        let counter = 1;
        while (true) {
    ...
    determineThemeCategory(content, title) {
    ...
    async getLastSyncStatus() {
        try {
            ...
            const wpPosts = await db.select({ count: sql `count(*)` }).from(posts)
