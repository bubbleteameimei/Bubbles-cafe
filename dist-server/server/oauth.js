...
export function setupOAuth(app) {
    ...
    app.post('/api/auth/social-login', async (req, res) => {
        try {
            ...
            if (user) {
                ...
                const oauthData = userMetadata.oauth || {};
    ...
    app.get('/api/auth/connections', isAuthenticated, (req, res) => {
        try {
            const user = req.user;
            const metadata = (user?.metadata || {});
    ...
    app.post('/api/auth/disconnect', isAuthenticated, async (req, res) => {
        try {
            const user = req.user;
            ...
            const currentOauth = (currentMetadata.oauth || {});
    ...
    app.patch('/api/auth/profile', isAuthenticated, async (req, res) => {
        try {
            ...
            if (req.session && req.session.user) {
                ...
                if (metadata) {
                    ...
                    if ((metadata.fullName !== undefined || metadata.displayName !== undefined)) {
                        ...
                        const newFullName = updatedMetadata.fullName ||
                            updatedMetadata.displayName;
                    ...
                    if ((metadata.avatar !== undefined || metadata.photoURL !== undefined)) {
                        ...
                        const newAvatar = updatedMetadata.avatar ||
                            updatedMetadata.photoURL;
                        // Only update if changed
                    ...
                    // Update bio
                    if (metadata.bio !== undefined && updatedMetadata.bio !== sessionUser.bio) {
                        sessionUser.bio = updatedMetadata.bio;
                    }
            ...
            // Type assertion to avoid TypeScript errors
            const typedMetadata = safeMetadata;
            res.json({
                id: updatedUser.id,
    ...
    app.patch('/api/auth/profile-with-image', isAuthenticated, async (req, res) => {
        try {
            const user = req.user;
            ...
            req.on('end', async () => {
                try {
                    ...
                    const updatedMetadata = updatedUser.metadata || {};
                    // Type assertion to avoid TypeScript errors
                    const typedMetadata = updatedMetadata;
                    res.json({
                        success: true,
                        user: {
                            id: updatedUser.id,
