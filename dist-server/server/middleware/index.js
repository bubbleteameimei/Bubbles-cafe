...
import { queryPerformanceMiddleware, wrapDbWithProfiler, optimizedQuery, queryProfiler } from './query-performance';
// Re-export all middleware
export { cacheControlMiddleware, imageOptimizationMiddleware, responseTimeMiddleware, responseTimeData, queryPerformanceMiddleware, wrapDbWithProfiler, optimizedQuery, queryProfiler };
/**
 * Apply all performance middleware in the recommended order
 */
export function applyPerformanceMiddleware(app, dbInstance) {
  ...
  <body>
    ...
    <div class="card">
      ...
      <table>
        ...
        <tbody>
          ${responseTimeReport.slowestRoutes.map((route) => `
            <tr>
              <td>${route.route}</td>
    ...
    <div class="card">
      ...
      <ul>
        ...
        ${responseTimeReport.slowestRoutes.some((r) => r.averageResponseTime > 1000) ? '<li class="error">Some routes have very high response times. Review implementation.</li>' : ''}
      </ul>
    </div>
