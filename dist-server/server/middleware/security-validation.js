...
export const securitySchemas = {
    ...
    userInput: z.object({
        ...
        id: z.union([z.string(), z.number()]).transform(val => typeof val === 'string' ? parseInt(val, 10) : val).refine(val => Number.isInteger(val) && val > 0, 'Invalid ID')
    }),
...
export async function validateSession(req, res, next) {
    ...
    if (req.session.fingerprint && req.session.fingerprint !== currentFingerprint) {
        ...
        req.session.destroy((err) => {
            if (err)
                securityLogger.error('Failed to destroy hijacked session', { error: err.message });
        });
    ...
    if (sessionAge > maxAge) {
        ...
        req.session.destroy((err) => {
            if (err)
                securityLogger.error('Failed to destroy expired session', { error: err.message });
        });
...
export function sanitizeInput(schema) {
    return (req, res, next) => {
        try {
...
function sanitizeQueryParams(query) {
    ...
    for (const [key, value] of Object.entries(query)) {
        if (typeof value === 'string') {
            ...
            sanitized[key] = value.map(v => typeof v === 'string' ? v.replace(/[<>'"&]/g, '').slice(0, 1000) : v);
        }
        else {
            sanitized[key] = value;
        }
...
export function limitRequestSize(maxSize = 10 * 1024 * 1024) {
    return (req, res, next) => {
