...
export class PostService {
    async getPost(id, includeContent = true) {
        try {
            const selectFields = {
                id: posts.id,
                title: posts.title,
    ...
    async getPosts(options = {}) {
        const { page = 1, limit = 10, category, search, authorId, includeSecret = false, sortBy = 'newest' } = options;
        ...
        try {
            ...
            if (search) {
                conditions.push(or(like(posts.title, `%${search}%`), like(posts.content, `%${search}%`), like(posts.excerpt, `%${search}%`)));
            }
            if (authorId)
    ...
    async updatePost(id, postData) {
        try {
            ...
            const existingMetadata = existingPost.metadata || {};
            const mergedMetadata = postData.metadata ? { ...existingMetadata, ...postData.metadata } : existingMetadata;
    ...
    async getPostCount(includeSecret = false) {
        try {
    ...
    async getRelatedPosts(postId, limit = 5) {
        try {
            ...
            const relatedPosts = await db.select({
                ...
                .from(posts)
                .where(and(eq(posts.isSecret, false), sql `${posts.id} != ${postId}`, or(eq(posts.themeCategory, currentPost.themeCategory || ''), eq(posts.authorId, currentPost.authorId))))
    ...
    async updatePostReaction(postId, isLike, increment = true) {
        try {
