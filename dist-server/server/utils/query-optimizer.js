 ...
 */
export async function cachedQuery(queryKey, queryFn, ttl = 300 // 5 minutes default
) {
 ...
 */
function updateQueryStats(queryKey, executionTime) {
 ...
 */
export async function optimizedBulkInsert(table, records, batchSize = 100) {
    if (!records.length)
        return 0;
    let inserted = 0;
    ...
    for (let i = 0; i < totalRecords; i += batchSize) {
        ...
        try {
            ...
            const placeholders = batch.map((_, index) => `(${columns.map((_, colIndex) => `$${index * columns.length + colIndex + 1}`).join(',')})`).join(',');
 ...
 */
export async function resilientQuery(queryFn, maxRetries = 3, baseDelay = 100) {
    let attempt = 0;
    let lastError = null;
 ...
 */
export function cleanQueryStats(maxEntries = 100) {
    const keys = Object.keys(queryStats);
    if (keys.length <= maxEntries)
        return;
    // Sort by last executed time and keep only the most recent ones
    const sortedKeys = keys.sort((a, b) => queryStats[b].lastExecuted.getTime() - queryStats[a].lastExecuted.getTime());
