...
>(({ className, ...props }, ref) => (
	<SheetPrimitive.Overlay
		className={cn(
			"fixed inset-0 z-50 bg-background/60 backdrop-blur-[2px] data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 lg:hidden",
			className
		)}
		{...props}
		ref={ref}
...
const sheetVariants = cva(
	"fixed z-50 gap-4 bg-background/95 p-6 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-200 data-[state=open]:duration-300 flex flex-col",
	{
		variants: {
			side: {
...
>(({ side = "right", className, children, ...props }, ref) => {
	...
	const close = () => {
		// Scope closing to this specific sheet by using its aria-labelledby value
		const labelledBy = (props as any)['aria-labelledby'] || defaultTitleId;
		const container = document.querySelector(`[role="dialog"][aria-labelledby="${labelledBy}"]`) as HTMLElement | null;
		if (!container) return;
		const localCloseBtn = container.querySelector('button[aria-label="Close"]') as HTMLButtonElement | null;
		if (localCloseBtn) {
			localCloseBtn.click();
			return;
		}
	...
	// Forward the ref without mutating it
	...
	return (
		<SheetPortal>
			...
			>
				{contentChildren}
				{/* Hidden close button for a11y and programmatic closing within this sheet */}
				<SheetPrimitive.Close aria-label="Close" className="sr-only" />
