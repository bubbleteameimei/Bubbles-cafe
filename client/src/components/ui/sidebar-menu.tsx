...
export function SidebarNavigation({ onNavigate }: { onNavigate?: () => void }) {
  ...
  React.useEffect(() => {
    // Only add touch events if mobile/tablet and sidebar is open
    if ((!sidebar?.isMobile && window.innerWidth <= 1024) || !sidebar?.openMobile) return;
  ...
  const handleNavigation = React.useCallback((path: string) => {
    ...
    try {
      ...
      // 3. Close the mobile/tablet sidebar immediately for instant feedback
      if (sidebar && (sidebar.isMobile || window.innerWidth <= 1024)) {
        sidebar.setOpenMobile(false);
      }
