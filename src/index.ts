...
router.get('/api/admin/wordpress/status', async (req: Request, env: Env) => {
  ...
  if (!env.SUPABASE_URL || !env.SUPABASE_ANON_KEY) {
    try {
      ...
      return json({
        ...
        nextSync: null,
        postsCount: 0,
        errors: lastStatus && lastStatus.startsWith('error:') ? [lastStatus] : [],
        totalProcessed: 0,
        syncInterval: 300000,
  ...
  try {
    ...
    const getSetting = (key: string) => settingsRows.find((s) => s && s.key === key);
    ...
    const intervalMsRaw = intervalSetting?.value ? parseInt(intervalSetting.value, 10) : NaN;
    const syncInterval =
      Number.isFinite(intervalMsRaw) && intervalMsRaw > 0 ? intervalMsRaw : 5 * 60 * 1000;

    let lastSync: string | null = null;
    let nextSync: string | null = null;

    const lastSyncMs = lastSyncSetting?.value ? parseInt(lastSyncSetting.value, 10) : NaN;
    ...
    const kvLastTimestamp = await env.SYNC_METADATA_KV.get('last_sync_timestamp');
    ...
    activityUrl.searchParams.set('select', 'id,action,details,created_at');
    ...
    if (actRes.ok) {
      ...
      if (Array.isArray(actRows) && actRows.length > 0) {
        ...
        if (status === 'running' && createdAt && Date.now() - createdAt.getTime() < 5 * 60 * 1000) {
          isRunning = true;
        }
      }
    }
    ...
    logsUrl.searchParams.set('select', 'id,action,details,created_at');
    ...
    if (logsRes.ok) {
      ...
      if (Array.isArray(logs)) {
        for (const row of logs) {
          ...
          const status = details?.status;
          if (status === 'error') {
            const msg = details?.message || details?.error || 'WordPress sync encountered an error';
            const ts =
              details?.finishedAt || details?.startedAt || row.created_at || row.createdAt || null;
            errors.push({
              id: String(row.id),
...
router.get('/api/admin/wordpress/logs', async (req: Request, env: Env) => {
  ...
  try {
    ...
    const logs = rows.map((row: any) => {
      ...
      const status =
        statusRaw === 'error' ? 'error' : statusRaw === 'running' ? 'running' : 'success';
...
router.post('/api/admin/wordpress/sync', async (req: Request, env: Env) => {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      const message = body?.error || body?.message || 'Failed to trigger WordPress sync';
...
router.post('/api/admin/wordpress/toggle', async (req: Request, env: Env) => {
  ...
  try {
    ...
    if (typeof body.enabled === 'boolean') {
      ...
      enabled = lowered === 'true' || lowered === '1' || lowered === 'yes' || lowered === 'on';
    }
  } catch {
        ...
        try {
          ...
          const payload = {
            ...
            totals: {
              ...
              dislikes:
                Number(reactions.baselineDislikes ?? 0) + Number(reactions.dislikesCount ?? 0),
            },
            ts: Date.now(),
          };
...
router.get('/api/themes/categories', async (_req: Request, env: Env) => {
  ...
  try {
    ...
    if (!res.ok) {
      return json({ categories: [], total: 0, source: 'supabase-error' }, { status: 500 });
    }
