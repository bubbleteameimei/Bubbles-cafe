...
export function responseTimeMiddleware(req: Request, res: Response, next: NextFunction) {
	...
	// Ensure we set headers when the response finishes
	res.on('finish', () => {
		...
		try {
			...
			// Server-Timing per spec: key;dur=value;desc="..."
			const existing = res.getHeader('Server-Timing');
			const timingPart = `app;dur=${responseTime.toFixed(2)}`;
			if (existing) {
				res.setHeader('Server-Timing', String(existing) + ', ' + timingPart);
			} else {
				res.setHeader('Server-Timing', timingPart);
			}
		} catch {}
	...
	// Provide a provisional header early; final precise time set on finish
	try {
		if (!res.headersSent) {
