 ...
 */
export const registerPaymentRoutes = (app) => {
    ...
    app.post('/api/payments/initialize', validateBody(initSchema), async (req, res) => {
        try {
            ...
            const response = await paystackService.initializeTransaction(amount, userEmail, txReference, callbackUrl, enhancedMetadata);
    ...
    app.get('/api/payments/verify/:reference', validateParams(verifyParamsSchema), async (req, res) => {
        try {
    ...
    app.post('/api/payments/_webhook-handler', async (req, res) => {
        try {
    ...
    app.get('/api/payments/plans', async (_req, res) => {
        try {
            ...
            try {
                const resp = await paystackService.listTransactions({ perPage: 1, page: 1 });
    ...
    app.get('/api/payments/subscription/status', async (req, res) => {
        try {
            ...
            try {
                const txList = await paystackService.listTransactions({ perPage: 10, page: 1, customer: userEmail, status: 'success' });
                const recent = (txList?.data || []).find((t) => t.customer?.email === userEmail && t.status === 'success');
                if (recent) {
