...
router.post('/pageview', async (req, res) => {
    try {
        const { path, referrer, timestamp, userAgent, screenWidth, screenHeight } = req.body;
        // Basic validation
        if (!path) {
            return res.status(400).json({
                error: 'Missing required fields',
...
router.post('/interaction', async (req, res) => {
    try {
        ...
        void storage.storePerformanceMetric({
            ...
            url: path || req.headers.referer || 'unknown',
...
router.get('/reading-time', async (req, res) => {
    try {
        ...
        const avgReadTime = Number(analyticsSummaryRaw?.avgReadTime ?? 180);
        const totalViewsBase = Number(analyticsSummaryRaw?.totalViews ?? 1000);
...
router.get('/devices-test', async (req, res) => {
    try {
        ...
        const totalSessions = Number(analytics?.totalViews ?? 1281);
        const baseTotals = {
 ...
 */
router.get('/reading-time-test', async (req, res) => {
    try {
        // Get analytics summary with reading time data
        const analyticsSummaryRaw = await storage.getAnalyticsSummary();
        const avgReadTime = Number(analyticsSummaryRaw?.avgReadTime ?? 180);
 ...
 */
router.get('/engagement-test', async (req, res) => {
    try {
 ...
 */
router.get('/site-test', async (req, res) => {
    try {
        // Get analytics summary as base data
        const analyticsSummaryRaw = await storage.getAnalyticsSummary();
        const totalViewsBase = Number(analyticsSummaryRaw?.totalViews ?? 1281);
        ...
        const siteAnalytics = {
            ...
            avgReadTime: Number(analyticsSummaryRaw?.avgReadTime ?? 171),
 ...
 */
router.get('/device-distribution-test', async (req, res) => {
    try {
        // Get analytics summary as base data
        const analyticsSummaryRaw = await storage.getAnalyticsSummary();
