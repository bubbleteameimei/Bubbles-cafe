...
```
Changelog:
- July 28, 2025. ✅ **ENHANCED CANVAS-BASED BLOOD DRIPPING EFFECT WITH ALTERNATING MODES** - Upgraded from CSS-based blood drops to sophisticated canvas-based particle system for "CAFE" text. Features alternating blood modes every 10 seconds: short (thin 0.2-0.8px drops, fast gravity 0.12) and long (thicker 0.5-1.5px drops, slow gravity 0.05). Blood particles originate from within the text shape using pixel detection. Fixed double scrollbar issue with overflow-x: hidden. Canvas positioned correctly relative to text bottom. Preserves original font styling while providing cinematic blood dripping effect.
